...
- [ ] 根据当前已经掌握的数据，编写精度诅咒的可视化代码，看到在固定成功率下，所需数据量随着精度要求增长的曲线。
    - [ ] 从 wandb 获取所有的实验数据，获取某个 group 下所有的实验数据，包括每次实验的config，`eval/env_ind/success_once`, `eval/dataset_ind/mse`的数据，后面两个eval的数据取 best 的结果作为这次实验的结果
    - [ ] 根据洞口大小分组，每个洞口大小下绘制 评价指标随着 num_traj 变化的曲线，x 轴使用 num_traj 的对数坐标。
    - [ ] 对于每个组，使用线性回归拟合，得到拟合的参数和线性回归的回归系数，
