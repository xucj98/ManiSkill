...
- [x] 根据当前已经掌握的数据，编写精度诅咒的可视化代码，看到在固定成功率下，所需数据量随着精度要求增长的曲线。
    - [x] 从 wandb 获取所有的实验数据，获取某个 group 下所有的实验数据，包括每次实验的config，`eval/env_ind/success_once`, `eval/dataset_ind/mse`的数据，后面两个eval的数据取 best 的结果作为这次实验的结果
    - [x] 根据洞口大小分组，每个洞口大小下绘制 评价指标随着 num_traj 变化的曲线，x 轴使用 num_traj 的对数坐标。
    - [x] 对于每个组，使用线性回归拟合，得到拟合的参数和线性回归的回归系数，
    - [x] 根据线性回归的结果，分别估计50%, 70%, 90%成功率需要的数据量
    - [x] 根据各个洞口大小下 x% 成功率所需要的数据量，绘制 数据量 w.r.t. 洞口大小 的曲线（数据量使用对数坐标）
    - [x] 得到最终精度诅咒的scaling law
    
- [ ] 在Peg Ins的任务下，测试不同观测和不同模型下的精度诅咒的有效性。
    - [ ] ~~修改Peg Ins的成功率评价：插入即成功，当前需要移动到指定位置才算成功，对于较大的洞口这两者有显著区别~~
    - [ ] 增加腕部相机
        - [x] 默认开启数据集压缩（目前还不会删除原始的数据，待到充分测试后再删除原始数据）
        - [x] 可视化验证数据
        - [ ] Hole Clearance = 5mm, 7mm, 40mm 实验, Num Traj = 500, 2k, 8k 实验
        - [ ] Hole Clearance = 6mm, 10mm, 20mm 实验, Num Traj = 200, 1k, 4k 实验
        - [ ] 数据压缩后，删除原始数据
...
- [ ] **数据聚合与模型再训练 (Phase 1, 2, 5, 6)**
    ...
    - [x] 确保 `ImitationDataset` 支持数据聚合
    - [x] 确保 `odpc.training.trainer` 可以被 A-IL 循环调用进行再训练
- [ ] **机器人自主探索与数据记录 (Phase 3)**
    - [x] 实现策略在机器人上的自主部署与轨迹记录逻辑
