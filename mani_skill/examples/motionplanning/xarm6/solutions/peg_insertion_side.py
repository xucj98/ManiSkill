...
def solve(env: PegInsertionSideEnv, seed=None, debug=False, vis=False):
    ...
    grasp_pose1 = grasp_pose * (sapien.Pose([offset, 0, 0]))
    grasp_pose2 = grasp_pose * (sapien.Pose([-offset, 0, 0]))
    peg_tail = env.peg.pose * sapien.Pose([-offset, 0, 0])
    d = -1
    if torch.sum(torch.abs(peg_tail.p[0] - grasp_pose1.p)) < torch.sum(torch.abs(peg_tail.p[0] - grasp_pose2.p)):
        d = 1
    grasp_pose = grasp_pose * sapien.Pose([d * offset, 0, 0])
    ...
    insert_pose = env.goal_pose * env.peg.pose.inv() * reach_pose
    offset = 0.01 + env.peg_half_sizes[0, 0].item()
    pre_insert_pose = insert_pose * sapien.Pose([d * offset, 0, 0])
    res = planner.move_to_pose_with_screw(pre_insert_pose)
    if res == -1 or res[-1]['elapsed_steps'].item() > 350: return -1
    # refine the insertion pose
    for _ in range(3):
        delta_pose = env.goal_pose * sapien.Pose([-offset, 0, 0]) * env.peg.pose.inv()
    ...
    # Insert
    # -------------------------------------------------------------------------- #
    delta_pose = env.goal_pose * sapien.Pose([0.05, 0, 0]) * env.peg.pose.inv()
    insert_pose = delta_pose * pre_insert_pose
